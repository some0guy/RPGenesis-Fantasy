...
class ContextMenu:
    def __init__(self):
        ...
        self.open = False
        self.pos = (0,0)
    def show(self, pos: Tuple[int,int], items: List[Tuple[str, Callable[[], None]]]):
        self.pos = pos
        self.items = items
        self.rects = []
        self.open = True
    ...
    def handle(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            for r, (_, fn) in zip(self.rects, self.items):
                if r.collidepoint(event.pos):
                    fn()
                    self.hide()
                    return True
    ...
    def draw(self, surf):
        ...
        self.rects = []
        for i, (label, _) in enumerate(self.items):
            r = pygame.Rect(x, y + i*h, w, h)
            self.rects.append(r)
            pygame.draw.rect(surf, PANEL_BG, r, border_radius=6)
...
# -------------------- EditorScreen --------------------
    def _hovered_tile(self) -> Optional[Tuple[int,int]]:
        # Canvas bounds must match your draw_canvas canvas_rect
    ...
    def _draw_link_tooltip(self, surf):
        # Don’t show over modal or while dropdowns are open
        if self.note_modal_open or self.any_dropdown_open():
            return
        ...
        # We enforce a single link per tile—grab the first
        link = t.links[0]
        ...
        h = line_h * len(lines) + pad*2 + (len(lines)-1)*2  # tiny extra spacing

        # Place near cursor; clamp to window
        mx, my = pygame.mouse.get_pos()
        x0 = mx + 16
        y0 = my + 16
        sw, sh = surf.get_size()
...
class EditorScreen:
    def __init__(self, app, mapdata: MapData):
        ...
        self.ctx_target = None  # ("listbox", index) or ("tileinfo", index)
    ...
    def cycle_left_mode(self):
        ...
        label = {"select":"Mode: Select", "paint":"Mode: Paint"}[self.left_click_mode]
    ...
    def apply_resize(self):
        ...
        for y in range(new_h):
            ...
            for x in range(new_w):
                if y < self.map.height and x < self.map.width and y < len(self.map.tiles) and x < len(self.map.tiles[y]):
    ...
    def _rebuild_scroll_items(self):
        ...
        for i, e in enumerate(t.links):
            label = f" → {e.get('target_map','?')} #{e.get('target_entry','')}"
    ...
    def draw_canvas(self, surf):
        ...
        # icons overlay + colored dots
        for y in range(self.map.height):
            for x in range(self.map.width):
                t = self.map.tiles[y][x]
                r = self.tile_rect(x,y)
                cx, cy = r.center
                # --- colored dots (wrap: 3 per row, stay inside tile) ---
                dots = []
                has = set()
                for e in t.npcs:
                ...
                for k in order:
                    if k in has:
                        dots.append(TYPE_DOT_COLORS[k])

                if dots:
                    cols = 3  # <<— exactly three per row
                    pad  = max(2, self.tile_size // 16)
                    # start with a comfy size, then shrink if rows wouldn't fit
                    radius = max(2, self.tile_size // 8)
                    rows_needed = math.ceil(len(dots) / cols)

                    # make sure the dot grid fits vertically in the tile
                    def grid_height(r): return rows_needed * (2*r + pad) - pad
                    while radius > 2 and grid_height(radius) > (self.tile_size - 2*pad):
                        radius -= 1

                    # top-left origin for the grid (keeps a small margin)
                    x0 = r.x + pad + radius
                    y0 = r.y + pad + radius
    ...
    def draw_right_panel(self, surf):
        ...
        if self.selected and (0 <= self.selected[0] < self.map.width) and (0 <= self.selected[1] < self.map.height):
            ...
            draw_text(surf, f"({x},{y}) — {'Passable' if t.walkable else 'Impassable'}  {(' • ' + note_preview) if note_preview else ''}", (x0+110, y0), TEXT_DIM)
    ...
    def draw(self, surf):
        surf.fill(PAPER_BG)
        # draw in order: canvas, right panel, top bar, context menu, modal
        self.draw_canvas(surf)
        self.draw_right_panel(surf)
        self.draw_top_bar(surf)

        # context menu last (below modal)
        self.ctx.draw(surf)
    ...
    def handle(self, event):
        # context menu consumes clicks if open
        if self.ctx.open and self.ctx.handle(event):
            return
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:
            # in list_box area?
            if self.list_box.rect.collidepoint(event.pos) and not dropdown_open and self.category in ("NPCs","Items"):
                idx = self.list_box.index_at_pos(event.pos) if hasattr(self.list_box, "index_at_pos") else None
                if idx is not None:
                    self.list_box.selected = idx
                    self.ctx_target = ("listbox", idx)
                    self.ctx.show(event.pos, [("Add to Selected", self.add_selected_to_tile)])
                    return
            # tile info area (remove entries quickly)
            if self.scroll_list.rect.collidepoint(event.pos):
                idx = self.scroll_list.index_at_pos(event.pos)
                if idx is not None and self.selected:
                    x,y = self.selected
                    if 0 <= x < self.map.width and 0 <= y < self.map.height:
                        t = self.map.tiles[y][x]
                        # Build a flat index map
                        flat = [("npc", i) for i in range(len(t.npcs))] + \
                               [("item", i) for i in range(len(t.items))] + \
                               [("link", i) for i in range(len(t.links))]
                        if 0 <= idx < len(flat):
                            kind, inner_idx = flat[idx]
                            def remove_it():
                                if kind == "npc":  self._record_remove_list_entry(t.npcs, inner_idx, "rem_npc")
                                elif kind == "item": self._record_remove_list_entry(t.items, inner_idx, "rem_item")
                                else: self._record_remove_list_entry(t.links, inner_idx, "rem_link")
                            self.ctx.show(event.pos, [("Remove", remove_it)])
                            return
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and canvas_rect.collidepoint(event.pos):
            if self.left_click_mode == "select":
                ...
                if event.button in (1,3):
                    ...
                    to_walk = True if event.button == 3 else False
            ...
            # allow painting even if cursor temporarily leaves canvas; still draw when in bounds
            to_walk = True if self.painting_button == 3 else False
            self.paint_at(*event.pos, to_walkable=to_walk, batch=True)
