...
class ListBox:
    def __init__(self, rect, items=None):
        ...
        self.items = items[:] if items else []
               # scroll kept for future; simple list now
        self.scroll = 0
        self.selected = -1
        self.hover_index = -1
        self.item_height = 24
...
class EditorScreen:
    def __init__(self, app, mapdata: MapData):
        ...
        self.brush_target_walkable = True  # True for Walkable paint, False for Block
        self.btn_brush_toggle = Button((880, 12, 110, 28), "Brush: OFF", self.toggle_brush)
        self.btn_brush_walk   = Button((1000, 12, 80, 28), "Walk", lambda: self.set_brush(True))
        self.btn_brush_block  = Button((1085, 12, 80, 28), "Block", lambda: self.set_brush(False))
        ...
        self.context_buttons: List[Button] = []
        self.context_for_tile: Optional[Tuple[int,int]] = None

        # canvas panning
        self.panning = False
        self.pan_start = (0,0)
        self.left_dragging = False  # for brush
    ...
    def toggle_brush(self):
        ...
        self.btn_brush_toggle.text = "Brush: ON" if self.brush_enabled else "Brush: OFF"
    def set_brush(self, to_walkable: bool):
        self.brush_target_walkable = to_walkable

    def apply_brush_at_mouse(self, mx, my):
        t = self.screen_to_tile(mx, my)
        if not t: return
        x,y = t
        self.map.tiles[y][x].walkable = self.brush_target_walkable
        self.selected = t  # also select as visual feedback
    ...
    # ---------- context menu ----------
    def open_context(self, tile_xy: Tuple[int,int], at_screen_pos: Tuple[int,int]):
        self.context_open = True
        self.context_for_tile = tile_xy
        x,y = at_screen_pos
        width = 360
        height = 10
        margin = 8
        ...
        tx, ty = tile_xy
        t = self.map.tiles[ty][tx]
        items: List[Tuple[str, Optional[int], Optional[int], str]] = []  # (type, index, subtype_index, label)

        # List entries
        for i, e in enumerate(t.npcs):
            label = f"{ICON_NPC} {e.get('name','(unnamed)')} [{e.get('id','')}] <{e.get('subcategory','')}>"[:46]
            items.append(("npc", i, None, label))
        for i, e in enumerate(t.items):
            label = f"{ICON_ITEM} {e.get('name','(unnamed)')} [{e.get('id','')}] <{e.get('subcategory','')}>"[:46]
            items.append(("item", i, None, label))
        for i, e in enumerate(t.links):
            label = f"{ICON_LINK} → {e.get('target_map','?')} #{e.get('target_entry','')}"[:46]
            items.append(("link", i, None, label))

        # Build buttons
        self.context_buttons = []
        row_y = y + margin
        # header
        header = pygame.Rect(x, y, width, 28)
        self.context_rect = pygame.Rect(x, y, width, 28 + len(items)*(btn_h+6) + margin*2)
        # keep on screen
        screen_w, screen_h = 1280, 720
        if self.context_rect.right > screen_w: self.context_rect.x = screen_w - self.context_rect.w - 8
        if self.context_rect.bottom > screen_h: self.context_rect.y = screen_h - self.context_rect.h - 8

        # recompute row_y after potential reposition
        row_y = self.context_rect.y + 34

        def make_remove(kind, idx):
            def _inner():
                tx, ty = self.context_for_tile
                t = self.map.tiles[ty][tx]
                if kind == "npc":
                    if 0 <= idx < len(t.npcs): t.npcs.pop(idx)
                elif kind == "item":
                    if 0 <= idx < len(t.items): t.items.pop(idx)
                elif kind == "link":
                    if 0 <= idx < len(t.links): t.links.pop(idx)
                self.open_context((tx,ty), (self.context_rect.x, self.context_rect.y))  # refresh
            return _inner

        for (kind, idx, _, label) in items:
            label_rect = pygame.Rect(self.context_rect.x+10, row_y, width-90, btn_h)
            # remove button
            btn = Button((self.context_rect.x+width-70, row_y, 60, btn_h), "Remove", make_remove(kind, idx), danger=True)
    ...
    def draw(self, surf):
        ...
        # brush UI
        self.btn_brush_toggle.draw(surf); self.btn_brush_walk.draw(surf); self.btn_brush_block.draw(surf)
        ...
        if self.context_open and self.context_for_tile:
            r = self.context_rect
            pygame.draw.rect(surf, PANEL_BG, r, border_radius=8)
            pygame.draw.rect(surf, GRID_LINE, r, 1, border_radius=8)
            tx, ty = self.context_for_tile
            draw_text(surf, f"Tile ({tx},{ty})", (r.x+10, r.y+8), TEXT_MAIN, FONT_BOLD)
            # draw rows
            row_y = r.y + 34
            # rebuild labels for drawing so they match remove buttons
            t = self.map.tiles[ty][tx]
            items: List[Tuple[str,int,str]] = []
            for i, e in enumerate(t.npcs):
                items.append(("npc", i, f"{ICON_NPC} {e.get('name','(unnamed)')} [{e.get('id','')}] <{e.get('subcategory','')}>"))
            for i, e in enumerate(t.items):
                items.append(("item", i, f"{ICON_ITEM} {e.get('name','(unnamed)')} [{e.get('id','')}] <{e.get('subcategory','')}>"))
            for i, e in enumerate(t.links):
                items.append(("link", i, f"{ICON_LINK} → {e.get('target_map','?')} #{e.get('target_entry','')}"))
            for (kind, idx, label) in items:
                pygame.draw.rect(surf, PANEL_BG_DARK, (r.x+10, row_y, r.w-90, 24), border_radius=6)
                draw_text(surf, label[:50], (r.x+16, row_y+4))
                row_y += 30
            # draw Remove buttons
    ...
    def handle(self, event):
        ...
        self.btn_brush_toggle.handle(event); self.btn_brush_walk.handle(event); self.btn_brush_block.handle(event)
        ...
        # context menu buttons (always handle)
        if self.context_open:
            for btn in list(self.context_buttons):
                btn.handle(event)

        # canvas interactions
        canvas_rect = pygame.Rect(0,50,900,670)
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1 and canvas_rect.collidepoint(event.pos):
                self.left_dragging = True
                if self.brush_enabled:
                    self.apply_brush_at_mouse(*event.pos)
                else:
                ...
                # open context on current tile
                t = self.screen_to_tile(*event.pos)
                if t:
                    self.open_context(t, event.pos)
            ...
            if self.left_dragging and self.brush_enabled and canvas_rect.collidepoint(event.pos):
                self.apply_brush_at_mouse(*event.pos)
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3 and not canvas_rect.collidepoint(event.pos):
            # clicking elsewhere closes context
            self.close_context()

        # panning with right-drag outside context
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 2:
            if canvas_rect.collidepoint(event.pos):
                self.panning = True; self.pan_start = event.pos
