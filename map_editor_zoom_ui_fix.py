...
NPC_DIR  = os.path.join(DATA_DIR, "NPCs")   # exact case per repo
...
def read_json_any(path: str, default: Any) -> Any:
    try:
        ...
        print(f"[editor] Missing file: {path}")
        return default
...
class StartScreen:
    ...
    def draw(self, surf):
        ...
        draw_text(surf, "Choose a map to edit or create a new one.", (60, 70), TEXT_DIM)
    ...
    def handle(self, event):
        ...
        self.w_inp.handle(event)
        self.h_inp.handle(event)
        self.desc_inp.handle(event)

    def update(self, dt):
        self.name_inp.update(dt); self.w_inp.update(dt); self.h_inp.update(dt); self.desc_inp.update(dt)
    ...
    def create_map(self):
        name = self.name_inp.text.strip() or "Untitled"
        try: w = max(8, int(self.w_inp.text))
        except: w = GRID_W_DEFAULT
        try: h = max(8, int(self.h_inp.text))
        except: h = GRID_H_DEFAULT
        desc = self.desc_inp.text.strip()
        md = MapData.new(name, desc, w, h)
        # save file + manifest
        file_name = f"{name}.json"
        write_json(os.path.join(MAP_DIR, file_name), md.to_dict())
        obj = read_json_any(MANIFEST, {"maps": []}); maps = obj.get("maps", [])
        entry = {"file": file_name, "name": name, "description": desc, "width": w, "height": h}
        updated = False
        for i, m in enumerate(maps):
            if m.get("file")==file_name:
                maps[i] = entry; updated = True; break
        if not updated: maps.append(entry)
...
class EditorScreen:
    ...
    def add_selected_to_tile(self):
        """One-shot add. Does NOT arm the cursor for repeated placement."""
        ...
        # note: no drag/cursor state set → no accidental repeat adds
    ...
    def save(self):
        # updates meta + writes file + manifest
        self.map.name = self.name_inp.text.strip() or "Untitled"
        self.map.description = self.desc_inp.text.strip()
    ...
    def draw(self, surf):
        ...
        if self.selected:
            ...
            if t.links:
                ...
                for e in t.links[:6]:
                    draw_text(surf, f"- → {e.get('target_map','?')} #{e.get('target_entry','')}", (940, y0)); y0 += 18
    ...
    def handle(self, event):
        # top
        self.name_inp.handle(event); self.desc_inp.handle(event)
    ...
    def update(self, dt):
        ...
        if self.category == "Links":
            self.link_entry_inp.update(dt)
